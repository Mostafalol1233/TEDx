...
import { CalendarIcon, MapPinIcon, ShoppingCart, Ticket, ShirtIcon, Clock } from "lucide-react";
    ...
    <Card className="overflow-hidden hover:shadow-xl transition-all duration-300 group h-full flex flex-col">
      <div className="relative overflow-hidden">
        <img 
          ...
          alt={product.name} 
          className={`w-full ${isTicket ? 'h-52' : 'h-64'} object-cover transition-transform duration-700 group-hover:scale-110`} 
        />
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-70"></div>
        
        {isTEDx && (
          <div className="absolute top-3 left-3 bg-red-600 px-3 py-1 rounded-full text-white text-xs font-bold shadow-lg flex items-center">
        ...
        <div className="absolute top-3 right-3 flex flex-col gap-2">
          ...
          {product.stock && product.stock < 10 && !product.unlimited && (
            <Badge variant="destructive" className="shadow-lg px-3 py-1 text-xs rounded-full animate-pulse">
              <div className="flex items-center">
      ...
      <CardContent className="p-5 flex-grow flex flex-col">
        ...
        {isTicket ? (
          ...
          <div className="mb-4">
            ...
            <div className="flex flex-wrap gap-2">
              {sizes.map(size => (
                <span key={size} className="px-3 py-1 bg-gray-100 rounded-full text-xs font-medium text-gray-700 hover:bg-gray-200 transition-colors cursor-default">
                  {size.trim()}
                </span>
              ))}
            </div>
        ...
        <div className="flex justify-between items-center mt-auto pt-3 border-t">
          <div className="flex flex-col">
            <span className={`${isTEDx ? 'text-red-600' : 'text-primary'} font-bold text-xl`}>
